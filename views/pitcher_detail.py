...
from data_loader import (
    ...
    load_movement_scores, load_velocity_scores, load_effectiveness,
...
def lookup_velocity(pitcher_name):
    vl = load_velocity_scores()
    if vl is None:
        return None
    p_vl = vl[vl["player_name"] == pitcher_name]
    if len(p_vl) == 0:
        return None
    return p_vl.iloc[0]["trusted_velocity_score"]
...
velocity_value = lookup_velocity(pitcher)
...
col5.metric("Velocity", format_metric(velocity_value))
...
if paom_value is not None and "paom_confidence" in final_row.columns:
    ...
    component_confidences = {
        ...
        "Velocity": r.get("velocity_confidence")
    }
