...
Purpose:
--------
Orchestrates the full per-year pipeline (74 -> 75 -> 77 -> 78 -> 76)
across all six seasons (2020-2025), so this doesn't require manually
editing and re-running five separate scripts six times each.
...
78 (velocity) depends on 77's output -- 76 (effectiveness) only
needs 75's output directly, so it's placed last as a matter of
convenience, not a real dependency requirement.
"""
