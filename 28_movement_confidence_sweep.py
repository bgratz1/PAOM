"""
28_movement_confidence_sweep.py

Purpose:
--------
Movement's confidence formula (min(n_pitch_types/4, 1) -- a linear
ramp that hard-caps at 4 pitch types) was, like Effectiveness's and
Command's original formulas, never actually tested against
...
26/27, adapted for Movement's structure.

KEY DIFFERENCE FROM EFFECTIVENESS/COMMAND: n_pitch_types is a
small, discrete count (roughly 1-7 in practice), not a large
continuous count like swings or pitches. Grids are scaled down
accordingly -- there's no meaningful difference between k=750 and
k=1000 when the underlying variable only ranges 1-7.

Velocity is held at its OWN current default throughout (not swept
here) -- see 29_velocity_confidence_sweep.py for Velocity's own
sweep. Testing one component's parameter at a time, holding
everything else fixed, matches the same approach already used for
Effectiveness and Command.
...
- Recompute trusted_movement_score with the SAME blend formula,
  just a different confidence input (movement_score, the pre-
  shrinkage PCA-derived percentile, is unaffected -- only the
  shrinkage step changes)
- Merge with Effectiveness/Command/Velocity (held at CURRENT
...
Reported per candidate: CV R2 on both targets, Movement's own
...
disabling Movement's shrinkage entirely produced severe degeneracy,
see 09_movement_component.py's history).

Output:
-------
Printed sweep table only -- no file saved. Once a winner is picked,
update 09_movement_component.py's actual formula to match.
"""
...
# down accordingly from swings/pitches-based sweeps
...
# ============================================================
# LOAD RAW MOVEMENT DATA (form-independent)
# ============================================================

print("Loading movement component (pre-shrinkage values)...")

movement_raw = pd.read_csv(MOVEMENT_FILE)

required_cols = ["player_name", "movement_score", "n_pitch_types"]
missing = [c for c in required_cols if c not in movement_raw.columns]
if missing:
    raise ValueError(
        f"Missing required columns: {missing}. This script needs "
        f"09_movement_component.py's output with movement_score "
        f"(pre-shrinkage) and n_pitch_types preserved."
    )

print(f"Loaded {len(movement_raw):,} pitcher rows")


# ============================================================
# LOAD OTHER COMPONENTS (held fixed -- this sweep only touches Movement)
# ============================================================
...
def run_candidate(form_name, param_value, confidence_fn):
    ...
    """
    mv = movement_raw.copy()

    mv["confidence_score"] = confidence_fn(mv["n_pitch_types"])

    league_average = mv["movement_score"].mean()
    mv["trusted_movement_score"] = (
        mv["movement_score"] * (mv["confidence_score"] / 100)
        + league_average * (1 - mv["confidence_score"] / 100)
    )

    # degeneracy guardrail
    n_extreme = (
        (mv["trusted_movement_score"] <= DEGENERACY_EXTREME_THRESHOLD)
        | (mv["trusted_movement_score"] >= 100 - DEGENERACY_EXTREME_THRESHOLD)
    ).sum()

    mv_pitcher = mv[["player_name", "trusted_movement_score"]]

    combined = eff_pitcher.merge(mv_pitcher, on="player_name", how="inner")
    ...
    mv_coef_primary = primary_coefs.set_index("component").loc[
        "trusted_movement_score", "coefficient"
    ]
    mv_coef_secondary = secondary_coefs.set_index("component").loc[
        "trusted_movement_score", "coefficient"
    ]
    ...
    return {
        ...
        "avg_confidence": mv["confidence_score"].mean(),
        "n_pitchers_at_extreme": n_extreme,
        "primary_cv_r2": primary_r2,
        "secondary_cv_r2": secondary_r2,
        "mv_coef_primary": mv_coef_primary,
        "mv_coef_secondary": mv_coef_secondary,
        "sign_agrees": np.sign(mv_coef_primary) == np.sign(mv_coef_secondary)
    }
...
for saturation in LINEAR_CAP_GRID:
    ...
    print(
        ...
        f"mv_coef={result['mv_coef_primary']:.4f}, "
...
# secondary guardrail specific to Movement: check whether the winning
# parameter is at the EDGE of the tested range (as happened with
# Command's k=500) rather than a genuine interior optimum
grid_edges = {
...
if best_primary["parameter"] in (edge_min, edge_max):
    print(
        ...
        f"the grid further before trusting this as a real peak, the "
        f"same issue flagged in Command's sweep."
    )

stable_sign = results_df[results_df["sign_agrees"]]
print(
    f"\n{len(stable_sign)} of {len(results_df)} candidates showed "
    f"sign-stable Movement coefficients across both targets."
)
