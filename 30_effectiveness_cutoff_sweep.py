"""
30_effectiveness_cutoff_sweep.py

Purpose:
--------
Effectiveness's entry cutoffs (MIN_PITCHES=75 AND MIN_SWINGS=50)
were, like the original confidence formulas, round numbers chosen
once and never tested against alternatives. Effectiveness is the
LIMITING FACTOR for the whole pipeline -- every 10_paom_score.py
run this session has shown exactly 394 pitchers clearing
Effectiveness's threshold vs. 722 for Command/Movement/Velocity,
with "dropped 0" at the merge step every time, meaning Effectiveness's
cutoff alone determines paom_score's final population size.

WHY THIS SWEEP IS STRUCTURALLY DIFFERENT FROM THE CONFIDENCE SWEEPS:
a cutoff is binary, not continuous -- a pitcher either clears it and
enters the regression, or doesn't exist in the component at all.
This means changing the cutoff changes WHICH ROWS FEED THE
REGRESSION FIT ITSELF, not just how an already-fitted score gets
re-weighted afterward. So this sweep re-runs the FULL Effectiveness
pipeline (aggregation -> regression -> confidence -> rollup) for
each candidate, not just the final blending step the confidence
sweeps could get away with.

One-parameter-at-a-time, same approach as every other sweep this
session: MIN_PITCHES swept holding MIN_SWINGS at its current
default (50); MIN_SWINGS swept holding MIN_PITCHES at its current
default (75).

For each candidate, reported:
- Population size (pitch-type ROWS entering the regression, and
  final PITCHERS reaching paom_score) -- the COVERAGE side of the
  tradeoff, tracked explicitly as its own signal, not just an
  R2-derived side effect
- INTERNAL regression CV R2 (how well csw/chase/hard_hit/gb predict
  xwOBA, on just the rows that clear this candidate's cutoff) --
  a cutoff could hurt or help this fit's own quality, independent
  of anything downstream
- DOWNSTREAM paom-level CV R2 on both validation targets (same
  confidence-weighted CV-Ridge fit used everywhere else)
- Effectiveness's own coefficient and sign-agreement in that
  downstream fit
- Degeneracy guardrail (pitchers near the exact 0/100 extremes)

Output:
-------
Printed sweep table only -- no file saved. Once a decision is made,
update 07d_final_effectiveness.py's MIN_PITCHES/MIN_SWINGS to match.
"""
...
# current defaults: MIN_PITCHES=75, MIN_SWINGS=50
MIN_PITCHES_GRID = [20, 30, 40, 50, 60, 75]
MIN_SWINGS_GRID = [10, 15, 20, 25, 35, 50]
...
print("\n==============================")
print(f"Sweeping MIN_PITCHES (MIN_SWINGS held at 50): {MIN_PITCHES_GRID}")
print("==============================")

for min_pitches in MIN_PITCHES_GRID:
    result = run_candidate("MIN_PITCHES", min_pitches, min_pitches, 50)
    if result is None:
        print(f"MIN_PITCHES={min_pitches}: too few rows, skipped")
        continue
    results.append(result)
    print(
        f"MIN_PITCHES={min_pitches}: rows={result['n_rows']}, "
...
print("\n==============================")
print(f"Sweeping MIN_SWINGS (MIN_PITCHES held at 75): {MIN_SWINGS_GRID}")
print("==============================")

for min_swings in MIN_SWINGS_GRID:
    result = run_candidate("MIN_SWINGS", min_swings, 75, min_swings)
    if result is None:
        print(f"MIN_SWINGS={min_swings}: too few rows, skipped")
        continue
    results.append(result)
    print(
        f"MIN_SWINGS={min_swings}: rows={result['n_rows']}, "
        f"pitchers={result['n_pitchers']}, extreme={result['n_extreme']}, "
        f"internal_R2={result['internal_r2']:.3f}, "
        f"primary_R2={result['primary_cv_r2']:.3f}, "
        f"secondary_R2={result['secondary_cv_r2']:.3f}, "
...
baseline = results_df[
    ((results_df["param"] == "MIN_PITCHES") & (results_df["value"] == 75))
]

if len(baseline) > 0:
    b = baseline.iloc[0]
    print(
        f"\nCurrent default (MIN_PITCHES=75, MIN_SWINGS=50): "
...
print(
    f"\nBest primary-target CV R2: {best_primary['param']}={best_primary['value']} "
    f"(R2={best_primary['primary_cv_r2']:.3f}, pitchers={best_primary['n_pitchers']}, "
    f"extreme={best_primary['n_extreme']})"
)
print(
    f"Best secondary-target CV R2: {best_secondary['param']}={best_secondary['value']} "
...
for grid_name, grid_vals in [("MIN_PITCHES", MIN_PITCHES_GRID), ("MIN_SWINGS", MIN_SWINGS_GRID)]:
    subset = results_df[results_df["param"] == grid_name]
    if len(subset) > 0:
        best_in_group = subset.loc[subset["primary_cv_r2"].idxmax()]
        if best_in_group["value"] in (min(grid_vals), max(grid_vals)):
            print(
                f"\nNOTE: within {grid_name}'s sweep, the best primary-R2 "
                f"candidate ({best_in_group['value']}) sits at the EDGE "
                f"of the tested grid -- consider extending further."
            )
